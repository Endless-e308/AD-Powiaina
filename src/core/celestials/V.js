...
class VRunUnlockState extends GameMechanicState {
  ...
  get canBeReduced() {
    return this.completions < this.config.values.length && this.completions !== 0 &&
      new PowiainaNum(this.reduction).neq(this.config.maxShardReduction(this.conditionBaseValue));
  }
  ...
  tryComplete() {
    ...
    if (this.config.condition() && PowiainaNum.gte(value, playerData.runRecords[this.id])) {
    ...
    PowiainaNum.gte(playerData.runRecords[this.id], this.conditionValue)) {
