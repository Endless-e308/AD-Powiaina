 ...
 * @property {number | PowiainaNum} value The final effect value (boost to whatever)
...
class GlyphEffectConfig {
  ...
  * @param {(function(number, number): number) | function(number, number): PowiainaNum} [setup.effect] Calculate effect
  *  value from level and strength
  * @param {function(number | PowiainaNum): string} [setup.formatEffect] Format the effect's value into a string. Defaults
  *  to format(x, 3, 3)
  * @param {function(number | PowiainaNum): string} [setup.formatSingleEffect] Format the effect's value into a string, used
  *  for effects which need to display different values in single values versus combined values (eg. power effects)
  * @param {function(number | PowiainaNum): number | PowiainaNum} [setup.softcap] An optional softcap to be applied after glyph
  ...
  */
  constructor(setup) {
    ...
    /**
    * @type {(function(number, number): number) | function(number, number): PowiainaNum} Calculate effect
    *  value from level and strength
    */
    this.effect = setup.effect;
    /**
    * @type {function(number | PowiainaNum): string} formatting function for the effect
    ...
    */
    ...
    /** @type {function(number | PowiainaNum): string} See info about setup, above */
    ...
    */
    ...
    /**
    * @type {function(number | PowiainaNum): string} formatSecondaryEffect formatting function for
  ...
  compareValues(effectValueA, effectValueB) {
    const result = PowiainaNum.compare(effectValueA, effectValueB);
   ...
   */
  checkBiggerIsBetter() {
    const baseEffect = new PowiainaNum(this.effect(1, 1.01));
    const biggerEffect = new PowiainaNum(this.effect(100, 2));
  ...
  static checkInputs(setup) {
    ...
    if (typeof emptyCombine !== "number" && !(emptyCombine instanceof PowiainaNum)) {
  ...
  static setupCombine(setup) {
    ...
    if (emptyCombine instanceof PowiainaNum) {
      if (softcap === undefined) return effects => ({ value: combine(effects), capped: false });
      const neqTest = emptyCombine.value instanceof PowiainaNum ? (a, b) => a.neq(b) : (a, b) => a !== b;
