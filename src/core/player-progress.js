export class PlayerProgress {
  ...
  get isInfinityUnlocked() {
    ...
    // Thus, it needs to manually support "before" and "after" states by converting both to PowiainaNum.
    const infinityData = this._player.infinitied ? this._player.infinitied : this._player.infinities;
    return new PowiainaNum(infinityData).gt(0) || this.isEternityUnlocked;
  }

  get isEternityUnlocked() {
    // Similarly to above, player.eternities is a number pre-reality update and a PowiainaNum post-reality update
    return new PowiainaNum(this._player.eternities).gt(0) || this.isRealityUnlocked;
  }
