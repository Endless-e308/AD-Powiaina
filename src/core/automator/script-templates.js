...
export class ScriptTemplate {
   ...
   * result in unparseable garbage here. Numbers are formatted assuming they're integers, and PowiainaNums are formatted
   * with 2 decimal places (in scientific notation if above 1000)
   * @param {Number | PowiainaNum} num  Number to format, disregarding current notation settings
   ...
   * @param {PowiainaNum} value   Numerical value for autobuyer settings (assumed to be seconds in time)
   ...
   * @param {PowiainaNum} params.finalEP        EP value at which to stop looping the script and continue onward
   ...
   * @param {PowiainaNum} params.eternities           Eternity count at which to stop grinding and move on
   */
  templateGrindEternities(params) {
    ...
    this.lines.push(`auto infinity ${this.format(
      PowiainaNum.pow(gapToEternity, 1 / params.crunchesPerEternity))} x highest`);
   ...
   * @param {Boolean} params.treeNowait   Nowait param to be passed into storeTreeData()
   * @param {String} params.treeStudies   Study import param to be passed into storeTreeData()
   * @param {PowiainaNum} params.infinities   Infinity count at which to stop grinding and move on
